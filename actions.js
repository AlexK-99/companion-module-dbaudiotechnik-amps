			...
			callback: async (event) => {
				if(self.ready) {
					switch (event.options.power) {
						case 0:
							self.powerObj.SetPosition(0).then(() => {
						...
						break
					case 1:
						self.powerObj.SetPosition(1).then(() => {
						...
						break
					case -1:
						self.powerObj.GetPosition().then((v) => {
							switch (v.item(0)) {
								case 0:
									self.powerObj.SetPosition(1).then(() => {
										self.log("info", "Amp Power On!")
				...
				if(self.ready) {
					if(event.options.mute === -1) {
						self.muteObj.forEach((obj)=> {
							obj.SetState(Types.OcaMuteState.Muted).then(() => {
						...
						self.muteObj[event.options.mute].SetState(Types.OcaMuteState.Muted).then(() => {
							self.log("info", "Mute Ch: " + numberToChar(event.options.mute))
						}).catch((err)=> {
							self.log("error", err.toString())
						})
			...
			callback: async (event) => {
				if(self.ready) {
					if(event.options.unmute === -1) {
						self.muteObj.forEach((obj)=> {
							obj.SetState(Types.OcaMuteState.Unmuted).then(() => {
						...
						self.muteObj[event.options.unmute].SetState(Types.OcaMuteState.Unmuted).then(() => {
							self.log("info", "Unmute Ch: " + numberToChar(event.options.unmute))
						}).catch((err)=> {
							self.log("error", err.toString())
						})
			...
			callback: async (event) => {
				if(self.ready) {
					if(event.options.toggelmute === -1) {
						...
						self.muteObj[event.options.toggelmute].GetState().then((v)=> {
							if(v === Types.OcaMuteState.Muted) {
								self.muteObj[event.options.toggelmute].SetState(Types.OcaMuteState.Unmuted).then(() => {
								...
								self.muteObj[event.options.toggelmute].SetState(Types.OcaMuteState.Muted).then(() => {
